...
func i_cond_ret(op uint8, c *CPU) int {
	if insCond(op, c) {
		c.PC = c.Pop()
		return 11
	} else {
...
func i_cond_jmp(op uint8, c *CPU) int {
	addr := insArg16(c)
	if insCond(op, c) {
		c.PC = addr
	}
...
func i_cond_call(op uint8, c *CPU) int {
	addr := insArg16(c)
	if insCond(op, c) {
		c.Push(c.PC)
		c.PC = addr
...
func i_call(op uint8, c *CPU) int {
	addr := insArg16(c)
	c.Push(c.PC)
	c.PC = addr
...
// PCHL: 0xE9
func i_pchl(op uint8, c *CPU) int {
	c.PC = c.HL()
	return 5
}

// RST: 0xC7, 0xCF, 0xD7, 0xDF, 0xE7, 0xEF, 0xF7, 0xFF
func i_rst(op uint8, c *CPU) int {
