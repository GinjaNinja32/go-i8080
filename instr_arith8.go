...
// STC: 0x37
func i_stc(op uint8, c *CPU) int {
	c.Flags |= F_CARRY
	return 4
}

// RRC: 0x0F
func i_rrc(op uint8, c *CPU) int {
...
func i_cmc(op uint8, c *CPU) int {
	c.Flags ^= F_CARRY
	return 4
}

// ADD: 0x80 to 0x87
func i_add(op uint8, c *CPU) int {
	reg := insArg3(op)
